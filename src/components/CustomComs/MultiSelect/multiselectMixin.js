...
export default {
  ...
  methods: {
    ...
    reverseAll: function () {
      var _self = this;
      this.internalValue = this.options.filter(item=> {
        let findNoResult = true;
        for (let i = 0; i < _self.internalValue.length; ++i) {
          if (typeof item == 'object') {
            let keyMatched = true;
            for (let key in item) {
              if (item.hasOwnProperty(key)) {
                if (item[key] != _self.internalValue[i][key]){
                  keyMatched = false;
                }
              }
            }
