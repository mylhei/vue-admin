        ...
        <div class="dates-events">
          <div class="events-week" v-for="week in currentDates">
            <div class="events-day" v-for="day in week" track-by="$index"
              ...
              <p class="day-number">{{day.monthDay}}</p>
              <div class="event-box">
                <event-card :event="event" :date="day.date" :firstDay="firstDay" :key="'fc_'+Math.random()" v-for="event in day.events" v-show="event.cellIndex <= eventLimit" @click="eventClick">
                  <template scope="p">
          ...
          <div class="more-body">
            <ul class="body-list">
              <li v-for="event in selectDay.events"
                  ...
                  @click="eventClick(event, $event,selectDay).date">
                {{event.title}}
              </li>
            </ul>
          </div>
...
<script type="text/babel">
  ...
  export default {
    ...
    methods : {
      ...
      eventClick(event, jsEvent,day) {
        if (!event.isShow) return;

        jsEvent.stopPropagation();
        let pos = this.computePos(jsEvent.target);
        this.$emit('eventClick', event, jsEvent, pos , day);
      }
    },
    filters: {
